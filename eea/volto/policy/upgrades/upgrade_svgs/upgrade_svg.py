...
def upgrade_svgs(portal):
    """Upgrade all svgs to fix the width and height property"""
    i = 0
    for brain in portal.portal_catalog():
        obj = brain.getObject()
        if hasattr(obj, 'image') and hasattr(obj.image, '_width') and hasattr(obj.image, '_height'):
            ...
            if contentType == "image/svg+xml":
                ...
                modified(obj.image)
                i += 1
        if hasattr(obj, 'preview_image') and hasattr(obj.preview_image, '_width') and hasattr(obj.preview_image, '_height'):
            logger.info("Processing %s", obj.absolute_url())
